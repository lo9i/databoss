...
func (g *OrmPlugin) FileOf(fd *descriptor.FileDescriptorProto) *generator.FileDescriptor {
	//for _, file := range g.AllFiles().File {
	the_g := reflect.TypeOf(g)
	the_f, _ := the_g.FieldByName("allFiles")
	the_v := reflect.ValueOf(the_f)

	for _, file := range *(*[]*generator.FileDescriptor)(unsafe.Pointer(&the_v)) {
